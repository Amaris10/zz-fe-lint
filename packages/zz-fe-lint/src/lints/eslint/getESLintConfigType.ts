 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-zzz/index
 * @returns eslint-config-zzz/react
 * @returns eslint-config-zzz/typescript/index
 * @returns eslint-config-zzz/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-zzz/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
